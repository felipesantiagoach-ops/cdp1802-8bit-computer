; This source for FIG-FORTH 1802 has been modified to 
; match the following FIG FORTH supplied to me by Lee Hart:
; as paper listing "source file 17"
; changes will be tagged "HRJ" if possible
; Herb Johnson June 17 2016
;
...
;
;; HRJ Apr 2016 mods Herb Johnson for A18 cross-assembler
;; change .EQU to EQU, things like that
;; HRJ June 17 2016 mode Herb Johnson to match Lee Hart
;; listing "source file 17"
; reassembled for Elf II:
;              Richard M Cox
;              2640 Calle Abedul
...
;; HRJ commentary from Lee Hart List follows
; this implementation does not include
; required disk I/O, namely words
;  (LINE), .LINE, DRIVE...[many more words listed]
;  [list end ends with] ....LIST, INDEX, TRIAD
;
; the I/O vectors are pointing to 
; routines for UART I/O
; for other systems yo uwill need to change
; the pointers and write your own routines.
:
; function		pointer	routine
;			address	address
;
; EMIT		        054A		153D
; KEY			0552		155F
...
; CR			0576		154A
;
;; HRJ end commentary from Hart listing
;
;
...
;;HRJ listing has manual fix from oh to zero
	  DW BRCH - 9
ZBRCH:  DW $ + 2
        LDA R9
        BNZ NO
...
;;HRJ THESE I/O ROUTINES ARE MODIFIED IN Lee Hart SOURC       
        ;
        ; SEND ASCII TO TERMINAL
        ;
IEMIT:  DW REMIT           ;HRJ CONSOLE OUT ADDR GOES HERE
        DB $84,"EMI",$D4 ; EMIT
        DW CEX - 5
EMIT:   DW NEST            ; OUTPUT CHAR TO
        DW IEMIT           ; TERMINAL AND INCREMENT
        DW ONE             ; "OUT"
...
;;HRJ WORDS NOT IN Hart SOURCE, REMOVED: CSEND, CSEND1
;; WORDS MODIFIED FROM FIG-FORTH: KEY, QTERM, CR

        DB $83,"KE",$D9  ; KEY   READ KEYBOARD
        DW EMIT - 7
KEY:    DW RKEY          ;KEYBD IN ADDRESS GOES HERE 
        ;
        ; TEST FOR BREAK
        ;
...
VLIS2:  DW DUP
        ...
        ;
        ;
;HRJ Hart LISTING CHANGES MSG
        DB $87,"MESSAG",$C5 ; MESSAGE
        ;does not have a test for disk present
        DW VLIST - 8
MSG:    DW NEST
        DW PDQ
        DB $07," MSG # "
...
SSKP:   INC R9
        ...
        SEP RC
        ;
;;HRJ ROUTINES REMOVED: (LINE) 
;; AND OTHER DISK I/O ROUTINE ZYZZY 

;;HRJ TASK IS MODIFIED
        DB $84, "TAS",$CB  ;TASK
        DW STOD-7
        DW NEST
        DW SEMIS
...
WRMLP:  LDA R7
        ...
        ;
;
;;HRJ NEW WORDS FROM Hart LISTING
; STAND ALONE I/O VECTORS

REMIT:  SEX R2		;EMIT
CHROUT: INP 6		;UART STATUS
        SHL
        BNF CHROUT
        SEX R9
        INC R9
